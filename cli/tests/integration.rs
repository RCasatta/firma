...
fn integration_test() {
    ...
    let r2 = firma_2of2
        .offline_dice("r2", vec![2u32; 59], 20, None)
        .unwrap();
    let r_err = firma_2of2.offline_dice("r3", vec![0u32; 59], 20, None);
    assert_eq!(
        r_err.unwrap_err().to_string(),
        Error::DiceValueErr(0, 20).to_string()
    );
    let r3 = firma_2of2
        .offline_restore("r4", "xprv", &r1.key.xprv.to_string(), None)
        .unwrap();
    ...
    let print_a = firma_2of2.offline_print(psbt_file_str, None).unwrap();
    ...
    assert!(
        firma_2of2
            .offline_print(psbt_file_str, encryption_key)
            .is_err(),
        "print doesn't need encryption_key"
    );
...
impl FirmaCommand {
    ...
    pub fn offline_dice(
        ...
        let result = self.offline("dice", args, encryption_key);
        let value = map_json_error(result)?;
        let output = from_value(value).unwrap();
        Ok(output)
    }
    ...
    pub fn offline_print(
        &self,
        psbt_file: &str,
        encryption_key: Option<&[u8]>,
    ) -> Result<PsbtPrettyPrint> {
        let result = self.offline("print", vec![psbt_file], encryption_key);
        let value = map_json_error(result)?;
        let output = from_value(value)?;
        Ok(output)
    }
